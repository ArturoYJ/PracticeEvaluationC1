...
# Este script ejecuta 04_roles.sql con variables de entorno usando psql -v

echo "Ejecutando 04_roles.sql con variables de entorno..."
...
if [ "$ROLE_EXISTS" != "1" ]; then
    psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" \
        ...
        -f /docker-entrypoint-initdb.d/04_roles.sql
